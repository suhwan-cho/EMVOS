...
class Trainer(object):
    ...
    def cycle_dataset(self, mode):
        if mode == 'train':
            loader_vos = self.train_loader
            i = -1
            for vos_data in loader_vos:
                ...
                # model run
                i += 1
                vos_out = self.model(imgs, masks[:, :1], None)
            ...
            print('[total stat at ep{:04d}] loss: {:.5f}, iou: {:.5f}'.format(self.epoch, self.stats['loss'].avg, self.stats['iou'].avg))
        ...
        if mode == 'val':
            ...
            for video_name, video_parts in self.val_set.get_videos():
                for vos_data in video_parts:
                    ...
                    # inference
                    vos_out = self.model(imgs, given_masks, None)
                    res_masks = vos_out['masks'][:, 1:-1].squeeze(2)
                    gt_masks = masks[:, 1:-1]
                    _, L, H0, W0 = res_masks.shape
                    object_ids = numpy.unique(gt_masks.cpu()).tolist()
                    object_ids.remove(0)

                    all_res_masks = numpy.zeros((len(object_ids), L, H0, W0))
                    all_gt_masks = numpy.zeros((len(object_ids), L, H0, W0))
                    for k in object_ids:
            ...
            print('[acc at ep{:04d}] J&F score: {:.5f}\n'.format(self.epoch, final_mean))
            return final_mean
